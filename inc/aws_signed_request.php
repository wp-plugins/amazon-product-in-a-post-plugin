...
if(!function_exists('aws_signed_request')){
	/*
	original aws_signed_request code from http://mierendo.com/software/aws_signed_query/ Copyright (c) 2009 Ulrich Mierendorff
	Parameters:
	...
	*/
	...
	//do request
		...
		if(ini_get('allow_url_fopen')){$hasfgc =  '1';}else{$hasfgc = '0';}
		if(function_exists('curl_version')){$hascurl = '1';}else{$hascurl = '0';}
		if($apip_usefileget!='0'){ // if setting says use fopen
			if($hasfgc == '1'){ // and the server allows
				$usefgc  = 1; // use fopen
				$usecurl = 0; // don't use curl
			}elseif($hascurl == '1'){ // if server does not have fopen but has curl
		...
		}elseif($apip_usecurlget!='0'){ //if setting is curl
			if($hascurl == '1'){ // and the server allows
				$usefgc  = 0; // dont use fopen
				$usecurl = 1; // use curl
			}elseif($hasfgc == '1'){ // if server does not have curl but has fopen
				$usefgc  = 1; // use fopen
